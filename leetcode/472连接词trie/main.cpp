...
public:
	...
	void search(string& str, int p, int level, Ret& ret) {
		Node* now = root;
		if(p == (int)str.length() && level>1) ret.push_back(str);
		for(int pos, i=p, n=(int)str.length(); i<n; i++) {
			pos = str[i] - 'a';
			if(!now->next[pos]) return ;
			now = now->next[pos];
			if(now->isEnd)
				search(str, i+1, level+1, ret);
		}
	}
...
//	vector<string> v = { "cat","cats","catsdogcats","dog","dogcatsdog","hippopotamuses","rat","ratcatdogcat" };
	...
	Solution s;
	vector<string> rs = s.findAllConcatenatedWordsInADict(v);
	for(string str : rs)
		cout << str << endl;
	return 0;
}
